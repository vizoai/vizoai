...
const buildTags = (config: BuildTagsParams) => {
  ...
  if (config.languageAlternates?.length) {
    tagsToRender.push(
      ...config.languageAlternates.map((languageAlternate) => ({
        tagName: "link",
        rel: "alternate",
    ...
    if (config.openGraph.type) {
      ...
      if (type === "profile" && config.openGraph.profile) {
        ...
        if (config.openGraph.book.authors?.length) {
          tagsToRender.push(
            ...config.openGraph.book.authors.map((author) => ({
        ...
        if (config.openGraph.book.tags?.length) {
          tagsToRender.push(
            ...config.openGraph.book.tags.map((tag) => ({
              property: "book:tag",
              content: tag,
            })),
          );
        ...
        if (config.openGraph.article.authors?.length) {
          tagsToRender.push(
            ...config.openGraph.article.authors.map((author) => ({
        ...
        if (config.openGraph.article.tags?.length) {
          tagsToRender.push(
            ...config.openGraph.article.tags.map((tag) => ({
        ...
        if (config.openGraph.video.actors?.length) {
          for (const actor of config.openGraph.video.actors) {
            if (actor.profile) {
              tagsToRender.push({
        ...
        if (config.openGraph.video.directors?.length) {
          tagsToRender.push(
            ...config.openGraph.video.directors.map((director) => ({
        ...
        if (config.openGraph.video.writers?.length) {
          tagsToRender.push(
            ...config.openGraph.video.writers.map((writer) => ({
        ...
        if (config.openGraph.video.tags?.length) {
          tagsToRender.push(
            ...config.openGraph.video.tags.map((tag) => ({
              property: "video:tag",
              content: tag,
            })),
          );
  ...
  if (config.additionalMetaTags && config.additionalMetaTags.length > 0) {
    tagsToRender.push(
      ...config.additionalMetaTags.map(({ keyOverride, ...tag }) => ({
        ...tag,
      })),
    );
  }
